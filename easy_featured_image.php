...
function edit_featured_images($column_name, $id, $post_id) {
global $post;
    if( $column_name == 'featuredimage' && has_post_thumbnail()) {
        the_post_thumbnail( array(100,100) );
}
    if( $column_name == 'featuredimage'){
        echo "<div class=\"row-actions\"><span class='edit'>";
        echo '<a href="'. admin_url() . 'media-upload.php?post_id='.$post->ID.'&type=image&TB_iframe=1&width=640&height=500" target="_blank">' . __( 'Edit' ) . '</a>';
        echo "</span></div>";
}}
